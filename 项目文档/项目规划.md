...
## 项目环境要求
- 操作系统：Windows / Linux（当前环境为 Linux）。
- Python：建议 3.8 及以上。
- GPU：NVIDIA RTX 3090（Ampere），推荐启用 FP16；文本嵌入可用 BF16/FP32，LLM 采用 4/8-bit 量化。
- 内存：建议 ≥8GB；显存：建议 ≥12GB（CLIP ViT-L/14 更佳）。
- 依赖与组件：
  - 必需：`torch`（CUDA 版）、`sentence-transformers`、`open_clip_torch`、`chromadb`、`pymupdf`、`tqdm`、`pillow`
  - 可选：`paddleocr`（扫描件 OCR）、`qdrant-client`、`faiss-gpu`（高性能近邻）、`onnxruntime`、`ollama`（本地 LLM）
- 性能与兼容建议：
  - 设备检测：自动选择 `cuda`/`cpu`；3090 上启用 FP16 的 `open_clip`；批大小根据显存动态调节（CLIP 32–64，Sbert 64–128）。
  - 并发：解析（CPU）与嵌入（GPU）并行；启用 `torch.backends.cudnn.benchmark=True`（输入尺寸稳定时）。
  - 跨平台：统一使用 `pathlib` 与 `shutil`；避免硬编码分隔符；文件移动与索引元数据一致更新。
