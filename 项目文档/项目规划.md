...
主要分为根目录入口脚本、用户界面模块 (`UI/`) 和核心逻辑模块 (`agent/`)。以下是项目中每个 Python 文件的详细说明：
...
- **`config.py`**: 系统的全局配置文件。
    ...
    - 配置了使用的模型名称（默认文本模型 `paraphrase-multilingual-mpnet-base-v2`，图像模型 `ViT-L-14`）。
    - 自动配置 `HF_ENDPOINT` 以适应国内网络环境。
    - 包含了设备选择逻辑（CPU/CUDA）。
...
- **`paper_manager.py`**: 文献管理服务。
    ...
    - `batch_organize`: 批量整理指定目录下的所有 PDF 文件。**支持智能混合整理模式：优先匹配现有主题，未匹配则自动聚类。**
